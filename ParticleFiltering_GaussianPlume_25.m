...
for t = 1 : SL
    ...
    p = p / exp(lamda * 3600) + sqrt(Q) * randn  ;
    C = pArr(t - time) * exp(-(x + miu * 3600 * t)^2 / (2 * sigmax^2)) + sqrt(R) * randn;
   
    % Particle filter
    for i = 1 : N
        ppartminus(i) = ppart(i) / exp(lamda * 3600) + sqrt(Q) * randn;
        Cpart = ppartminus(i) * exp(-(x + miu * 3600 * t)^2 / (2 * sigmax^2)) + sqrt(R) * randn;
        vhat = Cpart - C;
...
xlswrite('2.5real.xls', pArract);
xlswrite('2.5estimate.xls', phatPartArr);